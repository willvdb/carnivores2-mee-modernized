"""Shared trusted native process/workspace mechanics; no edition policy."""
import hashlib
import json
import os
...
from .session_io import capture, persist, read_journal, safe_path, session_root, write_blobs
...
def trusted_engine(engine, digest, experimental):
    if experimental is not True:
        raise FrontendError('native execution requires the explicit experimental gate')
...
def execution_spec(root, pins, evidence, capability, timeout, *, kind, policy):
    if type(timeout) not in (int, float) or not 30 <= timeout <= 3600:
        raise FrontendError('developer native validation timeout must be 30..3600 seconds')
    disjoint(root, pins, evidence)
    # The mode-specific policy already validates these arguments. The slot is now supplied
    # exclusively by the versioned contract, never the legacy reg= parser.
    policy_args = policy['candidate_argv']
    if policy_args[0] != f"reg={pins['native_slot']}":
        raise FrontendError('native policy slot disagrees with native state')
...
def prepare(store, association, selection, engine, digest, experimental, timeout, probe, adapter):
    ...
    with store.lock():
        pins, blobs = adapter.native_pins(store, association, selection, probe)
        capability = adapter.query_contract(evidence)
        identity = new_id(); root = session_root(store, identity)
        spec = adapter.execution_spec(root, pins, evidence, capability, timeout)
        ...
        created = now()
        journal = {'schema_version': adapter.SCHEMA, 'id': identity, 'path_flavor': os.name,
            ...
            'capabilities': {'session_workspace': 'validated', 'engine_contract': capability,
                'synthetic_child_lifecycle': 'not-executed', adapter.LIFECYCLE: 'not-executed',
...
def preflight(store, root, journal, probe, authorization, adapter):
    ...
    expected = adapter.execution_spec(root, pins, evidence, adapter.query_contract(evidence), spec.get('timeout_seconds'))
...
def run(store, identity, engine, digest, experimental, probe, cancel, adapter):
    from .session_runner import run_session
    journal = read_journal(store, identity)
    if adapter_for(journal) is not adapter:
        raise FrontendError('native command does not match the prepared session kind')
