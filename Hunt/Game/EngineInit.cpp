...
void SubmitDinoScore (int cindex) {
	...
	const auto st = Platform::LocalTime();
	...
	Characters[cindex].tempDate = (st.year << 20) + (st.month << 10) + st.day;
	Characters[cindex].tempTime = (st.hour << 10) + st.minute;
...
void InitEngine()
{
  ...
  if( Heap == nullptr )
  {
    Platform::ShowMessage("Error", "Error creating heap.");
    return;
  }
...
static void LoadConfig()
{
  ...
  char* ctx = nullptr;
  char* line = LegacyText::Token(text.data(), "\r\n", &ctx);
  while (line) {
    // Skip comments and empty lines
    if (line[0] == '#' || line[0] == '\0') {
      line = LegacyText::Token(nullptr, "\r\n", &ctx);
      continue;
    }
