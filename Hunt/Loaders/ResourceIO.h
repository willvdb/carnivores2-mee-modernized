...
inline bool Read(Platform::FileHandle file, int& out)
{
...
bool ReadRecord(Platform::FileHandle file, Runtime& out, Decoder decode)
{
    std::array<std::uint8_t,N> bytes; Value value;
    if (!ReadExact(file,bytes.data(),static_cast<std::uint32_t>(N)) || !decode(bytes.data(),N,value)) return false;
    ToRuntime(value,out); return true;
}
}
inline bool Read(Platform::FileHandle file, int (&out)[3][3])
{ return detail::ReadRecord<LegacyResource::ColorTableSize,LegacyResource::ColorTable>(file,out,LegacyResource::DecodeColors); }
inline bool Read(Platform::FileHandle file, TObjInfo& out)
...
inline bool ReadTexture(Platform::FileHandle file, unsigned short* out, std::size_t capacity, std::size_t words)
{
    ...
    while (words) {
        ...
        if (!ReadExact(file,bytes.data(),static_cast<std::uint32_t>(count*2)) ||
...
inline bool ReadPCM16(Platform::FileHandle file, short* out, std::size_t capacity, std::size_t length)
{
    ...
    while (length) {
        ...
        if (!ReadExact(file,bytes.data(),static_cast<std::uint32_t>(count)) ||
