...
inline bool ReadPixels(HANDLE file,WORD* out,std::size_t capacity,std::size_t count)
{
    static_assert((std::numeric_limits<WORD>::max)()>=UINT16_MAX,"Runtime pixel range");
    ...
    while(count) {
        ...
        if(!ReadExact(file,bytes.data(),static_cast<DWORD>(n*2)) ||
